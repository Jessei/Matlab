...
%initTour(ind) = [] ;
TTour(ind,:)=[];
%不应改变初始化的回路，所以在initTour中加入V0
%if ind == N
 %  initTour(ind) = V(1,1) ;
%else
 %  initTour1 = initTour(1,1:ind-1);
  % initTour2 = initTour(1,ind:N-1);
  % initTour = [initTour1,V(1,1),initTour2];
%end
vertex = TTour(:,1) ;
x1 = TTour(:,2) ;
y1 = TTour(:,3) ; 
...
while n <= N
    ...
    for i = 1:N-1
        ...
        if k ~= Fd & k ~= V0 & k ~= V0pre & k ~= V0suc & TEmin > ed(pre,V0)+ed(V0,suc)-ed(pre,k)-ed(k,suc)-ed(V0,V0pre)-ed(V0,V0suc)      % & ed(V0,V0pre)+ed(V0,V0suc)-ed(pre,V0)-ed(V0,suc)> 0 
            TEmin = ed(pre,V0) + ed(V0,suc) - ed(pre,k) - ed(k,suc)-ed(V0,V0pre)-ed(V0,V0suc) ;
            V1 = k ;
            V1pre = pre ;
            V1suc = suc ;
            loc = i ;%记录V1在数组中的位置
...
text(x2,y2,str) ;
text(V(1,2),V(1,3),num2str(V(1,1))) ;
title(['NodeEjection end and NodeEject num:  ',num2str(n-1)]);
%---------------构成回路的两种策略 策略2----------------------------
% V插入一条边中
TEmin = TEmax;
for i = 1:N-1
...
%----------------------edges ejection--------------------------------------
%1.寻找边max(e断开-e增加），并形成回路加一条边的状态 
%2.1 在边上操作，寻找与1,3近的点加入边中（可以先加入，再替换到最小）
%3.1 在边上增加点
%4.1 构成回路
%2.2 在回路中操作
%加边，删边操作实际就是对矩阵中的点进行重新排序操作
